...
from battery_status_tui.graph import (
    ...
    HISTORY_LOOKBACK_SECONDS, HISTORY_SECONDS, MAX_SPAN_SECONDS, MIN_EARLY_SLOPE,
    ...
    _chart_rows_and_percentages, _early_raster, _fill_chars, _forecast_span_columns,
    _history_column, _sleep_columns,
...
# Forecast spanning the right half of the graph places NOW at NOW_INDEX.
...
CENTER = Estimate((GRAPH_WIDTH - 1 - NOW_INDEX) * COLUMN_SECONDS, "pin")
...
class GraphTests(unittest.TestCase):
    ...
    def test_visible_span_is_fifteen_hours_forty_on_a_twenty_minute_grid(self):
        self.assertEqual(TIME_COLUMNS, 47)
        ...
        self.assertEqual(MAX_SPAN_SECONDS, 15 * 3600 + 40 * 60)
        self.assertEqual(GRAPH_WIDTH, 48)  # 47 time cells plus the NOW marker
        self.assertEqual(NOW_INDEX, 23)  # default projection origin only
        self.assertEqual(graph_module.GRAPH_ROWS, 3)
        self.assertEqual(HISTORY_LOOKBACK_SECONDS, MAX_SPAN_SECONDS + COLUMN_SECONDS)
    ...
    def test_visible_history_moves_at_each_live_projection_boundary(self):
        ...
        expected = {(22, 19): 17, (22, 20): 16, (22, 39): 16, (22, 40): 15}
        for (hour, minute), occupied_column in expected.items():
            with self.subTest(hour=hour, minute=minute):
                now = stamp(hour, minute)
                top, middle, bottom = chart_rows(sample(now), history, CENTER, now)
                occupied = [index for index in range(NOW_INDEX)
                            if top[index] != " " or middle[index] != " " or bottom[index] != " "]
    ...
    def test_current_bucket_is_hidden_until_it_closes(self):
        ...
        before_top, before_middle, before_bottom = chart_rows(sample(stamp(22, 19)), history, CENTER, stamp(22, 19))
        after_top, after_middle, after_bottom = chart_rows(sample(stamp(22, 20)), history, CENTER, stamp(22, 20))
    ...
    def test_now_column_is_exclusively_the_marker(self):
        ...
        top, middle, bottom = chart_rows(current, history, estimate, now, [sleep])
    ...
    def test_unclosed_sleep_bucket_is_not_moved_into_history(self):
        now = stamp(22, 19)
        sleep = SleepInterval(stamp(22, 10), stamp(22, 18), pre_percentage=80, post_percentage=79)
        top, middle, bottom = chart_rows(sample(now), [], None, now, [sleep])
    ...
    def test_axis_ticks_and_labels_follow_the_twenty_minute_projection(self):
        ...
        expected = {
            stamp(23): (list(range(2, GRAPH_WIDTH, 3)), "  16 17 18 19 20 21 22 23 00 01 02 03 04 05 06"),
            stamp(23, 20): (list(range(1, GRAPH_WIDTH, 3)), " 16 17 18 19 20 21 22 23 00 01 02 03 04 05 06 07"),
            stamp(23, 40): (list(range(0, GRAPH_WIDTH, 3)), "16 17 18 19 20 21 22 23 00 01 02 03 04 05 06 07"),
            midnight: (list(range(2, GRAPH_WIDTH, 3)), "  17 18 19 20 21 22 23 00 01 02 03 04 05 06 07"),
        }
    ...
    def test_axis_omits_partial_labels_and_admits_complete_right_label(self):
        self.assertNotIn("07", axis_rows(stamp(23))[1])
        self.assertTrue(axis_rows(stamp(23, 20))[1].endswith("07"))
        self.assertFalse(axis_rows(stamp(23, 20))[1].startswith("16"))
    ...
    def test_every_complete_hour_uses_its_real_position(self):
        ...
        self.assertEqual(axis[position], "┬")
        self.assertEqual(axis.count("┬"), 16)
    ...
    def test_now_marker_forecast_and_title_arrow_share_column(self):
        ...
        top, middle, bottom = chart_rows(self.current, self.history, estimate, self.now)
    ...
    def test_now_column_tracks_the_forecast_horizon(self):
        ...
        cases = [
            ...
            ("discharging-long", Measurement(now, 80, "discharging", False), Estimate(8 * 3600, "t"), 24),
        ...
        for name, current, estimate, span in cases:
            with self.subTest(case=name):
                ...
                top, middle, bottom, _ = _chart_rows_and_percentages(current, history, estimate, now)
                self.assertEqual((top[marker], middle[marker], bottom[marker]), ("│", "│", "│"))

                def braille(index):
                    glyphs = (top[index] + middle[index] + bottom[index]).strip()
    ...
    def test_braille_fill_height_tracks_forecast_percentage(self):
        for percentage in (0, 25, 50, 75, 100):
            with self.subTest(percentage=percentage):
                top, middle, bottom = _braille_fill(percentage)
                dots = sum((ord(character) - 0x2800).bit_count()
                           for character in (top, middle, bottom) if character != " ")
                self.assertEqual(dots // 2, max(1, round(percentage / 100 * 12)))

    def test_low_braille_subcolumns_keep_one_dot_including_zero(self):
        for percentage in (0, 1, 5, 10):
            with self.subTest(percentage=percentage):
                top, middle, bottom = _braille_fill(percentage)
                self.assertEqual((top, middle, bottom), (" ", " ", "⣀"))
    ...
    def test_sleep_residual_transfer_is_sleep_only_and_keeps_percentages(self):
        ...
        with patch.object(graph_module, "_sleep_residual_transfer",
            ...
            baseline_top, baseline_middle, baseline_bottom, baseline_percentages = _chart_rows_and_percentages(
        ...
        self.assertNotEqual((top, middle, bottom),
    ...
    def test_discharge_forecast_ends_at_the_predicted_empty_time(self):
        ...
        self.assertEqual((top[-1], middle[-1], bottom[-1]), (" ", " ", "⣀"))  # empty still shows a bottom dot
    ...
    def test_near_zero_discharge_forecast_keeps_bottom_dots(self):
        ...
        marker = viewport(current, estimate)
        top, middle, bottom, percentages = _chart_rows_and_percentages(current, [], estimate, self.now)
        ...
        self.assertEqual(middle[marker + 1:], " " * (GRAPH_WIDTH - marker - 1))
    ...
    def test_exact_zero_discharge_forecast_keeps_bottom_dots(self):
        ...
        self.assertEqual((top[marker + 1:], middle[marker + 1:], bottom[marker + 1:]),
                         (" ", " ", "⣀"))
    ...
    def test_long_discharge_forecast_is_capped_at_the_graph_midpoint(self):
        ...
        marker = viewport(current, estimate)
        top, middle, bottom, percentages = _chart_rows_and_percentages(current, history, estimate, self.now)
        ...
        self.assertGreater(forecast[-1], 15)
    ...
    def test_title_arrow_stays_above_the_capped_now_column(self):
        ...
        self.assertEqual(marker, NOW_INDEX)
        top, middle, bottom = chart_rows(current, self.history, estimate, self.now)
        self.assertEqual((top[marker], middle[marker], bottom[marker]), ("│", "│", "│"))
    ...
    def test_midpoint_clip_does_not_change_the_textual_eta(self):
        current = Measurement(self.now, 60, "discharging", False, power_w=6.5)
        estimate = Estimate(8 * 3600 + 40 * 60, "t")  # ceil = 26 columns, capped to the right half
        marker = now_column(current, estimate)
        self.assertEqual(marker, NOW_INDEX)
        rendered = plain(render_dashboard(current, self.history, None, estimate, self.now))
        eta_line = rendered.splitlines()[2]
        self.assertIn("8h40m", eta_line)  # full ETA, unclipped
        self.assertNotIn("~", eta_line)
        ...
        _, _, _, percentages = _chart_rows_and_percentages(current, self.history, estimate, self.now)
        self.assertGreater(percentages[-1], 0)
    ...
    def test_full_battery_on_ac_leaves_the_whole_width_for_history(self):
        ...
        top, middle, bottom, percentages = _chart_rows_and_percentages(current, self.history, None, self.now)
    ...
    def test_ac_state_rebuilds_forecast_for_the_current_direction(self):
        ...
        _, _, _, charging_percentages = _chart_rows_and_percentages(charging, [], estimate, self.now)
        ...
        _, _, _, discharge_percentages = _chart_rows_and_percentages(discharging, [], estimate, self.now)
        ...
        _, _, _, recharging_percentages = _chart_rows_and_percentages(charging, [], estimate, self.now)
    ...
    def test_history_columns_use_their_aggregated_percentages(self):
        ...
        top, middle, bottom, percentages = _chart_rows_and_percentages(self.current, history, CENTER, self.now)
        expected = {
    ...
    def test_observed_zero_is_visible_and_distinct_from_unknown(self):
        ...
        self.assertEqual((top[zero_column], middle[zero_column], bottom[zero_column]),
                         (" ", " ", "▁"))
    ...
    def test_known_low_solid_history_is_visible_with_actual_soc_color(self):
        for percentage, expected in ((0, "▁"), (1, "▁"), (2, "▁"), (3, "▁"),
                                     (5, "▁"), (50, "█")):
            with self.subTest(percentage=percentage):
                self.assertEqual(_fill_chars(percentage)[2], expected)
                column = project_column(stamp(20, 40), self.now)
                top, middle, bottom, percentages = _chart_rows_and_percentages(
                    self.current, [sample(stamp(20, 40), percentage)], CENTER, self.now
                )
                self.assertEqual(bottom[column], expected)
                self.assertEqual((top[column], middle[column], bottom[column]),
                                 _fill_chars(percentage))
                self.assertEqual(percentages[column], percentage)
                self.assertIn(
    ...
    def test_charging_forecast_uses_changing_gradient_colors(self):
        ...
        marker = viewport(current, Estimate(6 * 3600, "test"))
        forecast = percentages[marker + 1:]
    ...
    def test_discharging_forecast_uses_changing_gradient_colors(self):
        ...
        styled = _style_battery(top, percentages) + _style_battery(bottom, percentages)
        marker = viewport(current, estimate)
        forecast = [value for value in percentages[marker + 1:] if value is not None]
        visible_forecast = [value for value in forecast if round(value / 100 * 12) > 0]
        ...
        for column, percentage in enumerate(percentages[marker + 1:], marker + 1):
            if percentage is not None:
                elapsed = column_timestamp(column, self.now, marker) + COLUMN_SECONDS / 2 - self.now
    ...
    def test_battery_colors_preserve_width_now_axis_and_sleep(self):
        ...
        self.assertEqual(lines[3][GRAPH_OFFSET + marker], "│")
        self.assertEqual(lines[4][GRAPH_OFFSET:GRAPH_OFFSET + GRAPH_WIDTH],
                         axis_rows(self.now, marker)[0])
    ...
    def test_pre_sleep_sleep_and_post_resume_segments_all_remain_visible(self):
        ...
        top, middle, bottom, percentages = _chart_rows_and_percentages(self.current, history, CENTER, self.now, [sleep])
        columns = [_history_column(stamp(hour, minute), self.now, NOW_INDEX)
                   for hour, minute in ((17, 40), (18, 0), (19, 40), (20, 0), (20, 40))]
        self.assertNotEqual((top[columns[0]], middle[columns[0]], bottom[columns[0]]),
                            (" ", " ", " "))
        self.assertTrue(all(0x2800 <= ord(character) <= 0x28ff
                            for character in bottom[columns[1]:columns[2] + 1]))
        self.assertGreater(percentages[columns[1]], percentages[columns[2]])
        for column in columns[3:]:
            self.assertNotEqual((top[column], middle[column], bottom[column]), (" ", " ", " "))
    ...
    def test_short_sleep_below_fifty_uses_one_braille_column(self):
        ...
        top, middle, bottom, percentages = _chart_rows_and_percentages(self.current, [], CENTER, self.now, [sleep])
    ...
    def test_pre_sleep_sample_in_same_bucket_does_not_block_braille(self):
        ...
        top, middle, bottom = chart_rows(self.current, [sample(stamp(20, 4), 60)], CENTER, self.now, [sleep])
    ...
    def test_post_resume_sample_in_same_bucket_does_not_block_braille(self):
        sleep = SleepInterval(stamp(20, 5), stamp(20, 15), pre_percentage=60, post_percentage=58)
        top, middle, bottom = chart_rows(self.current, [sample(stamp(20, 16), 58)], CENTER, self.now, [sleep])
        column = project_column(stamp(20), self.now)
        self.assertTrue(all(0x2800 <= ord(character) <= 0x28ff for character in
                            (top[column], bottom[column]) if character != " "))
    ...
    def test_partial_sleep_boundary_does_not_overwrite_adjacent_history(self):
        ...
        top, middle, bottom = chart_rows(self.current, history, CENTER, self.now, [sleep])
    ...
    def test_rising_sleep_interpolation_follows_boundary_measurements(self):
        ...
        _, _, _, percentages = _chart_rows_and_percentages(self.current, [], CENTER, self.now, [sleep])
    ...
    def test_sleep_without_both_boundaries_remains_a_gap(self):
        ...
        top, middle, bottom = chart_rows(self.current, [], CENTER, self.now, [sleep])
    ...
    def test_measured_sample_inside_sleep_is_not_overwritten(self):
        ...
        top, middle, bottom = chart_rows(self.current, [measured], CENTER, self.now, [sleep])
        column = project_column(measured.timestamp, self.now)
        self.assertEqual((top[column], middle[column], bottom[column]), _fill_chars(80))
    ...
    def test_mixed_sleep_end_uses_active_data_for_right_subcolumn(self):
        ...
        top, middle, bottom = chart_rows(self.current, visible_history, CENTER, self.now, [sleep])
    ...
    def test_fully_sleeping_flat_bucket_keeps_sleep_geometry(self):
        ...
        top, middle, bottom = chart_rows(self.current, [], CENTER, self.now, [sleep])
        column = project_column(stamp(20), self.now)
        self.assertEqual((top[column], middle[column], bottom[column]), _braille_fill(67, 67))
    ...
    def test_sparse_active_history_after_sleep_does_not_bridge_empty_bucket(self):
        ...
        top, middle, bottom = chart_rows(self.current, [observed], CENTER, self.now, [sleep])
        ...
        self.assertEqual((top[measured], middle[measured], bottom[measured]), _fill_chars(8))
    ...
    def _row_cells(self, current, history, estimate, sleep, now):
        marker = now_column(current, estimate)
        top, middle, bottom, pct = _chart_rows_and_percentages(current, history, estimate, now, [sleep])
        cells = {}
        for column in range(marker):
            glyphs = top[column] + middle[column] + bottom[column]
            if not glyphs.strip():
                continue
            ...
            cells[column_timestamp(column, now, marker)] = (
                kind, top[column], middle[column], bottom[column],
    ...
    def test_very_low_soc_sleep_is_dark_red_braille_at_every_now_position(self):
        ...
        for estimate in (None, Estimate(50 * 60, "t"), Estimate(5 * 3600, "t")):
            with self.subTest(estimate=estimate):
                ...
                for kind, top_c, middle_c, bottom_c, value, colour in sleep_cells:
    ...
    def test_long_sleep_spanning_many_columns_slides_whole_and_clips_at_edges(self):
        ...
        wide_marker = now_column(current, None)
        mid_marker = now_column(current, Estimate(3 * 3600, "t"))
        wide = _sleep_columns(sleep, now, wide_marker)
        mid = _sleep_columns(sleep, now, mid_marker)
        self.assertGreaterEqual(len(wide), 24)
        self.assertGreaterEqual(len(wide), len(mid))
        # the surviving columns are the same wall-clock buckets, shifted left by the marker delta
        self.assertEqual([column_timestamp(c, now, wide_marker) for c in wide][-len(mid):],
                         [column_timestamp(c, now, mid_marker) for c in mid])
    ...
    def test_no_continuation_baseline_glyph_remains_anywhere(self):
        ...
        top, middle, bottom = chart_rows(self.current, history, Estimate(7200, "t"), self.now)
        self.assertNotIn("⠤", top + bottom)
    ...
    def test_pre_history_blank_keeps_now_marker_and_title_arrow_aligned(self):
        ...
        self.assertEqual(lines[3][GRAPH_OFFSET + marker], "│")
        self.assertEqual(lines[0].index("↓"), GRAPH_OFFSET + marker)
        for row in (lines[1], lines[2], lines[3]):
            self.assertEqual(row[GRAPH_OFFSET:GRAPH_OFFSET + marker].strip(" ▁▂▃▄▅▆▇█"), "")

    def test_dashboard_keeps_three_graph_rows_and_one_cell_margins(self):
        sleep = SleepInterval(stamp(18), stamp(19), pre_percentage=55, post_percentage=53)
        ...
        self.assertTrue(lines[1].startswith(" " * GRAPH_OFFSET))
        self.assertNotIn("1h00", lines[1])
        ...
        self.assertTrue(0x2800 <= ord(lines[3][GRAPH_OFFSET + sleep_column]) <= 0x28ff)
        self.assertEqual(lines[1][GRAPH_OFFSET - 1], " ")
        eta_start = graph_module.RIGHT_COLUMN_START
        self.assertEqual(lines[2][GRAPH_OFFSET + GRAPH_WIDTH], " ")
        self.assertEqual(lines[2][eta_start:], " 2h00m")
        self.assertEqual(lines[3][eta_start:], " empty")
        self.assertIn("┬", lines[4])
        self.assertRegex(lines[5], r"\d{2}")
        self.assertEqual(max(len(line) for line in lines), graph_module.RIGHT_COLUMN_END + 1)
    ...
    def test_meaning_labels_are_conditional(self):
        ...
        self.assertIn("empty", no_eta_meanings)
        ...
        self.assertEqual(charging_meanings[graph_module.RIGHT_COLUMN_START:], "  full")

    def test_power_values_remain_decimal_for_exact_and_approximate(self):
        cases = (
            (8.3, False, "8.3W ↓ 48%"),
            (10.8, False, "10.8W ↓ 48%"),
            (8.3, True, "~8.3W ↓ 48%"),
            (10.8, True, "~10.8W ↓ 48%"),
            (99.9, False, "99.9W ↓ 48%"),
            (100.0, False, "100.0W ↓ 48%"),
            (123.4, False, "123.4W ↓ 48%"),
            (123.4, True, "~123.4W ↓ 48%"),
        )
        decimal_columns = set()
        for watts, approximate, expected in cases:
            with self.subTest(watts=watts, approximate=approximate):
                ...
                value = f"{'~' if approximate else ''}{watts:.1f}"
                if len(value) < 5:
                    decimal_columns.add(rendered.index("."))
        missing = Measurement(self.now, 48, "discharging", False)
        self.assertIn("--W ↓ 48%", plain(title_line(missing)))

    def test_approved_title_status_variants(self):
        cases = (
            (Measurement(self.now, 88, "charging", True, power_w=13.5),
             "13.5W ↑ 88% P"),
            (Measurement(self.now, 90, "discharging", False, power_w=13.6,
                         power_approximate=True), "~13.6W ↓ 90% P"),
            (Measurement(self.now, 91, "discharging", False), "--W ↓ 91% P"),
            (Measurement(self.now, 100, "discharging", False, power_w=123.4,
                         power_approximate=True), "~123.4W ↓100% P"),
        )
        for current, expected in cases:
            with self.subTest(expected=expected):
                rendered = plain(title_line(current, "balanced"))
                self.assertTrue(rendered.endswith(expected))
                self.assertEqual(rendered.index("↑" if current.session_kind == "charging" else "↓"),
                                 GRAPH_OFFSET + NOW_INDEX)
                self.assertNotIn("SoC", rendered)

    def test_header_shows_integer_soc_without_profile(self):
        current = Measurement(self.now, 64.34, "discharging", False, power_w=10.9,
                              power_approximate=True)
        rendered = plain(title_line(current, "balanced"))
        self.assertIn("~10.9W ↓ 64% P", rendered)
        self.assertNotIn("SoC", rendered)
        self.assertEqual(rendered.index("↓"), GRAPH_OFFSET + NOW_INDEX)

    def test_soc_digit_boundaries_keep_downstream_title_columns_fixed(self):
        expected = {9: "9%", 10: "10%", 99: "99%", 100: "100%"}
        fixed_columns = set()
        for soc, percentage in expected.items():
            with self.subTest(soc=soc):
                current = Measurement(
                    self.now, soc, "discharging", False, power_w=10.8,
                )
                rendered = plain(title_line(current, "balanced"))
                separator = "" if soc == 100 else " "
                self.assertIn(f"10.8W ↓{separator}{percentage} P", rendered)
                arrow_i = rendered.index("↓")
                self.assertEqual(rendered.index(percentage),
                                 arrow_i + (1 if soc == 100 else 2))
                fixed_columns.add((arrow_i, rendered.index("."), rendered.index("W")))
        self.assertEqual(len(fixed_columns), 1)
        arrow_i, dot_i, watt_i = next(iter(fixed_columns))
        self.assertEqual(arrow_i, GRAPH_OFFSET + NOW_INDEX)

    def test_missing_profile_keeps_header_clean(self):
        rendered = plain(title_line(self.current))
        self.assertIn("8.4W ↓ 48%", rendered)
        self.assertNotIn("()", rendered)

    def test_power_profile_indicator_colors_are_the_agreed_xterm_values(self):
        self.assertEqual(graph_module.profile_color("power-saver"), "\x1b[38;5;238m")
        self.assertEqual(graph_module.profile_color("balanced"), "\x1b[38;5;244m")
        self.assertEqual(graph_module.profile_color("performance"), "\x1b[38;5;252m")
        self.assertEqual(graph_module.POWER_PROFILE_COLORS, {
            "power-saver": "\x1b[38;5;238m",
            "balanced": "\x1b[38;5;244m",
            "performance": "\x1b[38;5;252m",
        })

    def test_missing_or_unknown_profile_shows_no_label(self):
        self.assertIsNone(graph_module.profile_color(None))
        self.assertIsNone(graph_module.profile_color(""))
        self.assertIsNone(graph_module.profile_color("turbo-unknown"))
        for profile in (None, "", "turbo-unknown"):
            rendered = plain(title_line(self.current, profile))
            self.assertTrue(rendered.rstrip().endswith("48%"))
            self.assertNotIn("turbo-unknown", rendered)
            self.assertNotIn("eco", rendered)
            self.assertNotIn("normal", rendered)
            self.assertNotIn("turbo", rendered)
            self.assertNotIn("()", rendered)
            self.assertNotIn("🥵", rendered)
            self.assertNotIn("😎", rendered)
            self.assertNotIn("😴", rendered)

    def test_profile_indicator_sits_after_wattage_on_title(self):
        raw = title_line(self.current, "performance")
        self.assertTrue(plain(raw).endswith("8.4W ↓ 48% P"))
        self.assertIn("\x1b[38;5;252mP", raw)

    def test_title_arrow_aligns_with_now_marker_when_a_profile_is_present(self):
        ...
        self.assertLessEqual(graph_module.display_width(lines[0]), 64)

    def test_title_has_no_literal_soc_label(self):
        current = Measurement(self.now, 50, "discharging", False, power_w=9.0)
        rendered = plain(title_line(current, "balanced", NOW_INDEX))
        self.assertNotIn("SoC", rendered)
        self.assertIn("9.0W ↓ 50% P", rendered)

    def test_leftmost_history_column_uses_its_clock_aligned_bucket(self):
        # Mid-bucket NOW at the right edge: column 0 begins before now-MAX_SPAN.
        now = stamp(21, 10)
        current = sample(now)
        marker = now_column(current, None)
        self.assertEqual(marker, GRAPH_WIDTH - 1)
        left_edge = column_timestamp(0, now, marker)
        self.assertLess(left_edge, now - MAX_SPAN_SECONDS)
        self.assertLessEqual(now - HISTORY_LOOKBACK_SECONDS, left_edge)
        top, middle, bottom = chart_rows(current, [sample(left_edge, 70)], None, now)
        self.assertEqual((top[0], middle[0], bottom[0]), _fill_chars(70))
        # A genuine empty stretch older than the first sample stays blank.
        later = column_timestamp(2, now, marker)
        top, middle, bottom = chart_rows(current, [sample(later, 70)], None, now)
        self.assertEqual((top[0], middle[0], bottom[0]), (" ", " ", " "))

    def test_eta_groups_sit_on_the_middle_plotting_row(self):
        session = Session(1, "discharging", stamp(20), None, 55, None)
        estimate = Estimate(7200, "test")
        lines = plain(render_dashboard(
            self.current, self.history, session, estimate, self.now
        )).splitlines()
        eta_start = graph_module.RIGHT_COLUMN_START
        self.assertEqual(lines[1][:GRAPH_OFFSET], " " * GRAPH_OFFSET)
        self.assertNotIn("1h00", lines[1])
    ...
    def test_nearby_eta_block_has_fixed_edge_and_independent_target(self):
        self.assertEqual(GRAPH_OFFSET + GRAPH_WIDTH - 1, 53)
        self.assertEqual(graph_module.RIGHT_COLUMN_START, 54)
        self.assertEqual(graph_module.RIGHT_COLUMN_END, 59)
        self.assertEqual(graph_module.RIGHT_COLUMN_START - (GRAPH_OFFSET + GRAPH_WIDTH), 0)

        session = Session(1, "discharging", self.now, None, 91, None)
        discharge = Measurement(self.now, 91, "discharging", False)
        lines = plain(render_dashboard(
            discharge, self.history, session, None, self.now,
        )).splitlines()
        self.assertEqual(lines[2][graph_module.RIGHT_COLUMN_START:], "   n/a")
        self.assertEqual(lines[3][graph_module.RIGHT_COLUMN_START:], " empty")
        self.assertTrue(lines[2].startswith("0h00m"))
        self.assertTrue(lines[3].startswith("start"))

        charging = Measurement(self.now, 88, "charging", True, power_w=13.5)
        lines = plain(render_dashboard(
            charging, self.history, session, Estimate(19 * 60, "test"), self.now,
        )).splitlines()
        self.assertEqual(lines[2][graph_module.RIGHT_COLUMN_START:], " 0h19m")
        self.assertEqual(lines[3][graph_module.RIGHT_COLUMN_START:], "  full")

    def test_axis_ticks_and_hour_labels_use_footer_text_style(self):
        raw_lines = render_dashboard(
            self.current, self.history, None, None, self.now, health_percent=65.2,
        ).splitlines()
        axis, labels = axis_rows(self.now, GRAPH_WIDTH - 1)
        self.assertIn(" " * GRAPH_OFFSET + graph_module.MUTED + axis, raw_lines[4])
        self.assertIn(graph_module.MUTED + labels.ljust(GRAPH_WIDTH), raw_lines[5])
        self.assertIn(graph_module.MUTED + "SoH", raw_lines[5])
        self.assertIn(graph_module.MUTED + "65.2%", raw_lines[6])

    def test_wattage_has_one_separator_without_moving_arrow(self):
        current = Measurement(
            self.now, 60, "discharging", False, power_w=15.5,
        )
        rendered = plain(title_line(current, "performance"))
        arrow = rendered.index("↓")
        self.assertEqual(arrow, GRAPH_OFFSET + NOW_INDEX)
        self.assertIn("15.5W ↓ 60% P", rendered)
        self.assertEqual(rendered.index("15.5W"), arrow - len("15.5W") - 1)

    def test_maximum_wattage_fits_at_right_edge(self):
        current = Measurement(
            self.now, 60, "discharging", False, power_w=123.4,
            power_approximate=True,
        )
        rendered = plain(title_line(current, "performance", GRAPH_WIDTH - 1))
        self.assertEqual(rendered.index("↓"), GRAPH_OFFSET + GRAPH_WIDTH - 1)
        self.assertTrue(rendered.endswith("~123.4W ↓ 60% P"))
        self.assertLessEqual(len(rendered), 64)

    def test_subday_durations_include_minutes_unit(self):
        for seconds, expected in (
            (0, "0h00m"), (2 * 60, "0h02m"), (41 * 60, "0h41m"),
            (2 * 3600 + 48 * 60, "2h48m"),
            (5 * 3600 + 18 * 60, "5h18m"),
        ):
            self.assertEqual(graph_module.format_duration(seconds), expected)

    def test_endpoint_labels_use_shared_muted_style(self):
        session = Session(1, "discharging", stamp(20), None, 55, None)
        raw = render_dashboard(
            self.current, self.history, session, Estimate(3600, "test"), self.now,
        ).splitlines()[3]
        self.assertIn(graph_module.MUTED + "start", raw)
        self.assertIn(graph_module.MUTED, raw.rsplit("empty", 1)[0])
        self.assertNotIn(graph_module.DIM + "start", raw)

    def test_profile_indicator_follows_wattage(self):
        current = Measurement(self.now, 60, "discharging", False, power_w=15.5)
        midpoint = plain(title_line(current, None, NOW_INDEX))
        right_edge = plain(title_line(current, None, GRAPH_WIDTH - 1))
        self.assertNotEqual(midpoint.index("↓"), right_edge.index("↓"))
        self.assertNotEqual(midpoint.index("15.5W"), right_edge.index("15.5W"))
        self.assertNotEqual(midpoint.index("W"), right_edge.index("W"))
        for profile in ("performance", "balanced", "power-saver"):
            rendered = plain(title_line(current, profile, NOW_INDEX))
            self.assertTrue(rendered.endswith("15.5W ↓ 60% P"))

        no_eta = plain(render_dashboard(
            current, self.history, None, None, self.now, health_percent=65.2,
            power_profile="performance",
        )).splitlines()
        with_eta = plain(render_dashboard(
            current, self.history, None, Estimate(3 * 3600, "test"), self.now,
            health_percent=65.2, power_profile="performance",
        )).splitlines()
        self.assertNotEqual(no_eta[0].index("↓"), with_eta[0].index("↓"))
        self.assertTrue(no_eta[0].endswith("P"))
        self.assertTrue(with_eta[0].endswith("P"))

    def test_soh_moves_to_bottom_left(self):
        lines = plain(render_dashboard(
            self.current, self.history, None, None, self.now, health_percent=65.2
        )).splitlines()
        self.assertTrue(lines[5].startswith("SoH   "))
        self.assertEqual(lines[6], "65.2%")
    ...
    def test_health_is_fixed_after_graph_at_all_bucket_phases(self):
        for minute in (0, 20, 40):
            ...
            self.assertEqual(len(rendered), len(baseline) + 1)
            self.assertEqual(rendered[:5], baseline[:5])
            self.assertEqual(rendered[5][GRAPH_OFFSET:GRAPH_OFFSET + GRAPH_WIDTH],
                             baseline[5][GRAPH_OFFSET:GRAPH_OFFSET + GRAPH_WIDTH])
            self.assertTrue(rendered[5].startswith("SoH"))
            self.assertEqual(rendered[6], "62.3%")
